...
def create_bucket(bucket_name):
    ...
    if res['ResponseMetadata']['HTTPStatusCode'] == 200:
        print(f"Bucket {bucket_name} was created or already existing and owned by this user.")
        return
...
res = s3.put_bucket_notification_configuration(Bucket = bucket_base_name,
        NotificationConfiguration=bucket_notifications_configuration)

if res['ResponseMetadata']['HTTPStatusCode'] == 200:
    print(f"Notification for bucket: {bucket_base_name} was created or already existing.")
else:
    print(f"There was an error during creating notification for bucket: {bucket_base_name}: {res}")
    exit(1)